...
func init() {
	...
	// The MMemoryStoreConnectionPoolUsage requires a Count() type aggregation, as it grows forever.
	registerMetricView(MMemoryStoreConnectionPoolUsage, MMemoryStoreConnectionPoolUsage.Description(), view.Count(),
		[]tag.Key{KeyConnectionShard, KeyPoolUsageResult})

	// The MMemoryStoreConnectionPoolUsage requires a LastValue() type aggregation, as it's gauge metric.=
