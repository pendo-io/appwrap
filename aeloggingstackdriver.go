...
func getLogCtxVal(aeInfo AppengineInfo, hreq *http.Request, logger *logging.Logger, trace string) *loggingCtxValue {
	...
	return &loggingCtxValue{
		...
		hreq:   hreq,
		labels: map[string]string{
			"appengine_googleapis_com/instance_name": aeInfo.InstanceID(),
			"pendo_io/service":                       aeInfo.ModuleName(),
			"pendo_io/version":                       aeInfo.VersionID(),
			"pendo_io/request_host":                  hreq.Host,
			"pendo_io/request_method":                hreq.Method,
			"pendo_io/request_url":                   hreq.URL.String(),
			"pendo_io/remote_ip":                     remoteIp,
			"pendo_io/useragent":                     hreq.UserAgent(),
		},
		logger: logger,
		trace:  trace,
	}
