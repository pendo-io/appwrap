...
func LoadIndex(data []byte) (DatastoreIndex, error) {
	...
	index := make(DatastoreIndex, len(indexYaml.Indexes))
	for _, spec := range indexYaml.Indexes {
		if spec.Kind == "" {
		...
		entityIndex := entityIndex{ancestor: spec.Ancestor, fields: make(map[string]fieldIndex, len(spec.Properties))}
		for i, prop := range spec.Properties {
			if prop.Name == "" {
				...
				entityIndex.fields[prop.Name] = fieldIndex{
		...
		index[spec.Kind] = append(index[spec.Kind], entityIndex)
	}
