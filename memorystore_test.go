...
func (m *redisClientMock) Del(ctx context.Context, keys ...string) error {
	args := m.Called(ctx, keys)
	return args.Error(0)
}

func (m *redisClientMock) Exists(ctx context.Context, keys ...string) (int64, error) {
	args := m.Called(ctx, keys)
...
func (m *redisClientMock) FlushAllAsync(ctx context.Context) error {
	args := m.Called(ctx)
	return args.Error(0)
}

func (m *redisClientMock) Get(ctx context.Context, key string) ([]byte, error) {
	args := m.Called(ctx, key)
	return args.Get(0).([]byte), args.Error(1)
}

func (m *redisClientMock) IncrBy(ctx context.Context, key string, value int64) (int64, error) {
	args := m.Called(ctx, key, value)
	return args.Get(0).(int64), args.Error(1)
}

func (m *redisClientMock) MGet(ctx context.Context, keys ...string) ([]interface{}, error) {
	args := m.Called(ctx, keys)
...
func (m *redisClientMock) Watch(ctx context.Context, fn func(*redis.Tx) error, keys ...string) error {
...
func (m *redisPipelineMock) IncrBy(ctx context.Context, key string, value int64) {
	m.Called(ctx, key, value)
}

func (m *redisPipelineMock) Set(ctx context.Context, key string, value interface{}, expiration time.Duration) {
	m.Called(ctx, key, value, expiration)
}

func (m *redisPipelineMock) SetNX(ctx context.Context, key string, value interface{}, expiration time.Duration) {
	m.Called(ctx, key, value, expiration)
}
...
func (s *MemorystoreTest) newMemstore() (Memorystore, []*redisClientMock) {
	...
	m, err := ms.NewMemorystore(context.Background(), appInfo, "", "", 2)
	if err != nil {
		panic(err)
	}
...
func (s *MemorystoreTest) TestNewAppengineMemcacheThreadSafety(c *C) {
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func() {
			...
			<-startingLine
			mIntf, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 1)
			if err != nil {
				panic(err)
			}
...
func (s *MemorystoreTest) TestNewAppengineRateLimitedMemcache(c *C) {
	...
	memcache, err := ms.NewRateLimitedMemorystore(context.Background(), appMock, "cacheloc", "cachename", 1, log, createLimiter)
	c.Assert(err, IsNil)
...
func (s *MemorystoreTest) TestAPIConnectError(c *C) {
	...
	m, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 1)
	c.Assert(err, IsNil)
	apiMock.AssertExpectations(c)

	// Check that connFn is not called again, since setup was successful
	ms.connectFn = func(context.Context) (redisAPIService, error) { return nil, connErr }
	m2, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 1)
	c.Assert(err, IsNil)
...
func (s *MemorystoreTest) TestGetAddrKeepShards(c *C) {
	...
	mockShard(2, true)
	mockShard(3, false)
	_, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 4)
	...
	ms.addrDontRetryUntil = time.Time{} // allow retry now
	mockShard(1, true)
	m, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 4)
	c.Assert(err, IsNil)
	apiMock.AssertExpectations(c)

	// Ensure that there are no further calls to GetInstance
	m2, err := ms.NewMemorystore(context.Background(), appMock, "cacheloc", "cachename", 4)
	c.Assert(err, IsNil)
	c.Assert(&m2.(Memorystore).clients[0], Equals, &m.(Memorystore).clients[0]) // same underlying clients
...
func (s *MemorystoreTest) TestDoCompareAndSwap(c *C) {
	...
	// Item does not exist
	clientMocks[0].On("Get", mock.Anything, fullKey).Return(([]byte)(nil), redis.Nil).Once()
	err = ms.doCompareAndSwap(ms.c, item, clientMocks[0], fullKey)
	c.Assert(err, Equals, CacheErrNotStored)
	checkMocks()
...
func (s *MemorystoreTest) TestGet(c *C) {
	...
	// miss should convert error
	clientMocks[1].On("Get", mock.Anything, fullKey).Return([]byte(nil), redis.Nil).Once()
	item, err = ms.Get(key)
