...
func (ms Memorystore) Add(item *CacheItem) error {
	...
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/add")
	defer span.End()

	span.AddAttributes(trace.StringAttribute("key", fullKey))
	span.AddAttributes(trace.Int64Attribute("shard", int64(shard)))
...
func (ms Memorystore) AddMulti(items []*CacheItem) error {
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/add-multi")
	defer span.End()

	span.AddAttributes(trace.Int64Attribute("num-keys", int64(len(items))))

	addMultiForShard := func(shard int, itemIndices map[string]int, shardKeys []string) ([]redis.Cmder, error) {
		c, span := trace.StartSpan(c, "pendo.io/memorystore/add-multi-shard")
		defer span.End()

		span.AddAttributes(trace.StringAttribute("first-key", shardKeys[0]))
		span.AddAttributes(trace.Int64Attribute("num-keys", int64(len(shardKeys))))
		span.AddAttributes(trace.Int64Attribute("shard", int64(shard)))
...
func (ms Memorystore) CompareAndSwap(item *CacheItem) error {
	...
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/cas")
	defer span.End()

	span.AddAttributes(trace.StringAttribute("first-key", fullKey))
	span.AddAttributes(trace.Int64Attribute("shard", int64(shard)))
...
func (ms Memorystore) DeleteMulti(keys []string) error {
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/delete-multi")
	defer span.End()
	...
	for i, client := range ms.clients {
		...
		func() {
			c, span := trace.StartSpan(c, "pendo.io/memorystore/delete-multi-shard")
			defer span.End()

			span.AddAttributes(trace.StringAttribute("first-key", shardKeys[0]))
			span.AddAttributes(trace.Int64Attribute("num-keys", int64(len(shardKeys))))
			span.AddAttributes(trace.Int64Attribute("shard", int64(i)))
...
func (ms Memorystore) Get(key string) (*CacheItem, error) {
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/get")
	defer span.End()
...
func (ms Memorystore) GetMulti(keys []string) (map[string]*CacheItem, error) {
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/get-multi")
	defer span.End()

	span.AddAttributes(trace.Int64Attribute("num-keys", int64(len(keys))))

	getMultiForShard := func(shard int, itemIndices map[string]int, shardKeys []string) ([]interface{}, error) {
		c, span := trace.StartSpan(c, "pendo.io/memorystore/get-multi-shard")
		defer span.End()
...
func (ms Memorystore) Increment(key string, amount int64, initialValue uint64) (incr uint64, err error) {
	fullKey, shard := ms.namespacedKeyAndShard(key)
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/incr")
	defer span.End()
...
func (ms Memorystore) IncrementExisting(key string, amount int64) (uint64, error) {
	fullKey, shard := ms.namespacedKeyAndShard(key)
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/incr-existing")
	defer span.End()
...
func (ms Memorystore) Set(item *CacheItem) error {
	fullKey, shard := ms.namespacedKeyAndShard(item.Key)
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/set")
	defer span.End()
...
func (ms Memorystore) SetMulti(items []*CacheItem) error {
	c, span := trace.StartSpan(ms.c, "pendo.io/memorystore/set-multi")
	defer span.End()

	span.AddAttributes(trace.Int64Attribute("num-keys", int64(len(items))))

	setMultiForShard := func(shard int, itemIndices map[string]int, shardKeys []string) error {
		c, span := trace.StartSpan(c, "pendo.io/memorystore/set-multi-shard")
		defer span.End()
