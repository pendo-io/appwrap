...
func getRedisAddr(c context.Context, loc CacheLocation, name CacheName, shards CacheShards) []string {
	...
	for i := range redisAddrs {
		shardName := CacheName(fmt.Sprintf("%s-%d", name, i))
		instance, err := client.GetInstance(c, &redispb.GetInstanceRequest{
			Name: fmt.Sprintf("projects/%s/locations/%s/instances/%s", projectId, loc, shardName),
		})
		if err != nil {
			panic(err)
		}

		redisAddrs[i] = fmt.Sprintf("%s:%d", instance.Host, instance.Port)
	}
	return redisAddrs
}
...
func NewAppengineMemcache(c context.Context, loc CacheLocation, name CacheName, shards CacheShards) Memcache {
	if redisClients == nil {
		...
		if redisClients == nil {
			redisClients = make([]redisClientInterface, shards)
			...
			for i := range addrs {
				...
				redisClients[i] = &redisClientImplementation{client, client}
			}
		}
	}
...
type keyPositionFinder []map[string]int

func (kpf keyPositionFinder) getItemListIndex(shard int, namespacedKey string) int {
	return kpf[shard][namespacedKey]
}

func (ms Memorystore) shardedNamespacedKeysForItems(items []*CacheItem) (namespacedKeys [][]string, originalPositions keyPositionFinder) {
...
func (ms Memorystore) shardedNamespacedKeys(keys []string) (namespacedKeys [][]string, originalPositions keyPositionFinder) {
	namespacedKeys = make([][]string, len(ms.clients))
	originalPositions = make(keyPositionFinder, len(ms.clients))
	for i := range originalPositions {
		originalPositions[i] = make(map[string]int)
	}
	for i, key := range keys {
		namespacedKey, shard := ms.namespacedKeyAndShard(key)
		namespacedKeys[shard] = append(namespacedKeys[shard], namespacedKey)
		originalPositions[shard][namespacedKey] = i
	}
...
func (ms Memorystore) AddMulti(items []*CacheItem) error {
	...
	for shard := 0; shard < len(ms.clients); shard++ {
		...
		for _, key := range shardKeys {
			item := items[itemIndices.getItemListIndex(shard, key)]
		...
		for i, result := range results {
			...
			if added, err := result.(boolCmdInterface).Result(); err != nil {
				errList[itemIndices.getItemListIndex(shard, key)] = err
				haveErrors = true
			} else if !added {
				errList[itemIndices.getItemListIndex(shard, key)] = CacheErrNotStored
				haveErrors = true
			}
		}
	}
...
func (ms Memorystore) GetMulti(keys []string) (map[string]*CacheItem, error) {
	...
	for shard := 0; shard < len(ms.clients); shard++ {
		...
		for i, val := range vals {
			...
			key := keys[keyIndices.getItemListIndex(shard, shardKeys[i])]
