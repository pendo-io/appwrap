...
func NewCloudDatastore(c context.Context) (Datastore, error) {
	...
	return newCloudDatastore(c, dsClient, "", 5*time.Second), nil
}
...
func withDeadline(parent context.Context, time time.Time, f func(context.Context) error) error {
	...
	if err != nil && tctx.Err() != nil && parent.Err() == nil {
		// This means the individual datastore request timed out, but the parent request is still alive
		// So we can return a retryable error to the caller
		return status.Error(codes.DeadlineExceeded, "datastore timeout")
	}
	// otherwise, just return whatever error we got
	return err
}
...
func (cds CloudDatastore) AllocateIDSet(incompleteKeys []*DatastoreKey) ([]*DatastoreKey, error) {
	...
	err := withTimeout(cds.ctx, cds.timeout, func(tctx context.Context) error {
		var err error
	...
	err := withTimeout(cds.ctx, cds.timeout, func(tctx context.Context) error {
		...
		commit, err = cds.client.RunInTransaction(tctx, func(transaction *datastore.Transaction) error {
			ct := CloudTransaction{
				client:      cds.client,
				ctx:         tctx,
				deadline:    time.Now().Add(cds.timeout),
...
func (cds CloudDatastore) NewQuery(kind string) DatastoreQuery {
	q := CloudDatastoreQuery{ctx: cds.ctx, client: cds.client, deadline: time.Now().Add(cds.timeout), q: datastore.NewQuery(kind)}
	if cds.namespace != "" {
...
func (ct CloudTransaction) NewQuery(kind string) DatastoreQuery {
	q := CloudDatastoreQuery{ctx: ct.ctx, client: ct.client, deadline: ct.deadline, q: datastore.NewQuery(kind)}
...
func (cdq CloudDatastoreQuery) GetAll(dst interface{}) ([]*DatastoreKey, error) {
	...
	err := withDeadline(cdq.ctx, cdq.deadline, func(tctx context.Context) error {
		var err error
